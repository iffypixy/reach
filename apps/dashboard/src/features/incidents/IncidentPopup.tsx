...
export const IncidentPopupContent = ({ incident }: Props) => (
	<div className="min-w-56 space-y-2 text-sm">
		...
		{incident.address && <p className="text-xs text-slate-600">{incident.address}</p>}
		<div className="space-y-1.5 border-t border-slate-200 pt-2">
			<p className="text-xs font-medium text-slate-700">Inbound units</p>
			{incident.dispatchUnits.map((unit) => (
				<div key={unit.id} className="flex items-center gap-2 text-xs">
					...
					<span className="font-mono text-slate-500">{getUnitEtaLabel(unit)}</span>
				</div>
			))}
		</div>
